...
const pidsOf = (output) => (output || '').split('\n').map(Number).filter(Boolean);

// PIDs of Claude's main process (macOS / Linux). On Linux, matching on the full path
// would also match this script when it was given --app, so exclude ourselves.
function claudePids(app) {
  if (process.platform === 'darwin') {
    return pidsOf(spawnSync('pgrep', ['-x', macAppName(app)], { encoding: 'utf8' }).stdout);
  }
  return pidsOf(spawnSync('pgrep', ['-f', app], { encoding: 'utf8' }).stdout).filter((pid) => pid !== process.pid);
}
...
function isClaudeRunning(app) {
  ...
  return claudePids(app).length > 0;
}
...
async function quitClaude(app) {
  ...
  // Electron quits gracefully on SIGTERM, the same way as ⌘Q. AppleScript's "quit" would
  // instead need the macOS Automation permission and interrupt the user with a prompt.
  for (const pid of claudePids(app)) {
    try {
...
function launchClaude(app, port) {
  ...
  if (process.platform === 'darwin') {
    ...
    if (r.status !== 0) throw new Error(`无法启动 ${app}: ${(r.stderr || r.error?.message || '').trim()}`);
    return;
  }
...
// process.execPath has symlinks resolved, e.g. Homebrew's versioned Cellar path, which
// `brew upgrade` deletes. Prefer the `node` on PATH that points at the same binary.
...
NODE=''
# The node used at install time first, then the usual places in case it was upgraded or moved.
for candidate in ${shellQuote(stableNodePath())} /opt/homebrew/bin/node /usr/local/bin/node; do
...
if [ -z "$NODE" ]; then
  OUT='找不到 Node.js。请安装 Node.js 22 或更新版本，然后重新运行一次安装。'
elif OUT=$("$NODE" "$SCRIPT" start --restart --detach --port ${port} 2>&1); then
  exit 0
fi
...
function cmdInstall(opts) {
  ...
  if (process.platform === 'darwin') {
    ...
    fs.writeFileSync(cmd, `@echo off\r\n"${stableNodePath()}" "${script}" start --detach --port ${opts.port}\r\nif errorlevel 1 pause\r\n`);
    console.log(`已在桌面创建启动器: ${cmd}`);
    console.log('以后先从托盘完全退出 Claude，再双击它打开 Claude。');
  } else {
    console.log(`以后用这条命令打开 Claude:\n  ${shellQuote(stableNodePath())} ${shellQuote(script)} start --restart --detach`);
  }
}
